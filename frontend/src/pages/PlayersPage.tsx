...
export function PlayersPage() {
  ...
  // Maneja la rotación del rol entre player y captain mediante clics consecutivos
  function handleToggleRole(player: any) {
    ...
    const newRole = (player.role === 'player' || player.role === 'admin') ? 'captain' : 'player'

    if (newRole === 'captain') {
      const alreadyCaptain = teams?.some((t: any) => t.captain === player.user || t.captain === player.id)
      if (alreadyCaptain) {
        alert('Este jugador ya es capitán de un equipo. Solo puede ser capitán de un equipo a la vez.')
        return
      }
    }

    const confirmMessage = `¿Estás seguro de cambiar el rol de "${player.gamertag}" a ${newRole === 'captain' ? 'Capitán' : 'Jugador'}?`
              ...
              <tr key={p.id}>
                ...
                <td>
                  <span className={`role-badge ${currentRole}`} style={{
                    fontWeight: currentRole === 'captain' ? 'bold' : 'normal',
                    color: currentRole === 'captain' ? '#1890ff' : 'inherit'
                  }}>
                    {getRoleLabel(currentRole)}
                  </span>
                </td>
                <td>{teams?.find((t: any) => t.id === p.team)?.teamName || p.team || '-'}</td>
                ...
                <td>
                  <div style={{ display: 'flex', gap: '5px' }}>
                    ...
                    {isAdmin && !isSelf && (
                      ...
                      >
                        {(currentRole === 'player' || currentRole === 'admin') && 'Hacer Capitán'}
                        {currentRole === 'captain' && 'Hacer Jugador'}
                      </button>
                    )}
