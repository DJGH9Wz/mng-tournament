...
const { data: users } = useResourceList(USERS_RESOURCE)
  ...
  const [originalCaptainId, setOriginalCaptainId] = useState<number | null>(null)
  ...
  function handleSubmit() {
    ...
    if (editingId && originalCaptainId !== null && finalCaptainId !== originalCaptainId) {
      const oldName = users?.find((u: any) => u.id === originalCaptainId)?.gamertag || `Usuario #${originalCaptainId}`;
      const newName = users?.find((u: any) => u.id === finalCaptainId)?.gamertag || `Usuario #${finalCaptainId}`;
      if (!confirm(`¿Estás seguro de cambiar el capitán de "${oldName}" a "${newName}"?\n\nEl jugador "${oldName}" perderá el rol de capitán y ya no podrá gestionar este equipo.`)) {
        return
      }
    }
  ...
  function handleEdit(team: any) {
    ...
    setOriginalCaptainId(Number(team.captain))
    ...
    setForm({
      ...
      status: team.status,
      captain: Number(team.captain),
      members: currentMemberIds.filter((id: number) => id !== Number(team.captain)),
    })
  }
      ...
      {canUserCreateOrEdit ? (
        <div className="form-card">
          ...
          <div className="captain-selection-section" style={{ marginTop: '15px' }}>
            ...
            {editingId && originalCaptainId !== null && (
              <p style={{ fontSize: '0.85em', color: '#aaa', marginBottom: '8px' }}>
                Capitán actual: <strong>{users?.find((u: any) => u.id === originalCaptainId)?.gamertag || `Usuario #${originalCaptainId}`}</strong>
              </p>
            )}
            <select
              className="custom-input"
        ...
        <div className="form-card text-center text-muted">
          <p>Solo los administradores o el capitan de un equipo pueden realizar modificaciones.</p>
        </div>
      )}
